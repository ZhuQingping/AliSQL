...
int Log_event::apply_event(Relay_log_info *rli) {
  ...
  if (!(parallel = rli->is_parallel_exec()) ||
    ...
    if (myduck::duckdb_log_options & LOG_DUCKDB_MULTI_TRX_BATCH_DETAIL) {
      myduck::log_duckdb_apply_event_type(get_type_str());
    }
    if (rli->info_thd->get_duckdb_context(false) != nullptr &&
        rli->info_thd->get_duckdb_context()->need_implicit_commit_batch(this)) {
      if (rli->info_thd->get_duckdb_context()->implicit_commit_batch()) {
        return -1;
      }
    }
...
bool Query_log_event::write(Basic_ostream *ostream) {
  ...
  */
  ...
  if (thd && thd->variables.duckdb_data_import_mode) {
    *start++ = Q_DUCKDB_DATA_IMPORT_MODE;
    *start++ = 1;
  }
...
void Query_log_event::print_query_header(
  ...
  */
  ...
  if (duckdb_data_import_mode != print_event_info->duckdb_data_import_mode) {
    my_b_printf(file, "SET @@session.duckdb_data_import_mode=%d%s\n",
                duckdb_data_import_mode, print_event_info->delimiter);
  }
}
...
int Query_log_event::do_apply_event(Relay_log_info const *rli,
  ...
  if (strcmp("BEGIN", query) == 0 && thd->multi_trx_in_batch()) {
    return 0;
  }

  if (strcmp("COMMIT", query) == 0 &&
      rli->info_thd->get_duckdb_context(false) != nullptr &&
      thd->get_duckdb_context()->duckdb_delay_commit(this)) {
    return 0;
  }
...

  {
    ...
    if (ignored_error_code((expected_error)) ||
      ...
      */
      ...
      if (duckdb_data_import_mode != 0xff) {
        assert(duckdb_data_import_mode == 1);
        thd->variables.duckdb_data_import_mode = true;
      } else {
        thd->variables.duckdb_data_import_mode = false;
      }
...
int Rotate_log_event::do_update_pos(Relay_log_info *rli) {
  ...
  if (!is_relay_log_event() &&  /* not a relay log rotate */
      !is_artificial_event() && /* not a fake rotate */
      server_id != 0 /* not a rotate generated by slave */) {
    /* In scenarios with high write loads and costly batch commits, we don't
       want to trigger a commit on every binlog rotates. However, there's a
       problem: not committing when binlog rotations prevents relay log purging.
       Therefore, we added a '_frequency' like parameter to perform a commit
       every few binlogs, balancing performance and relay log purge. */
    if (rli->info_thd->multi_trx_in_batch() &&
        (rli->get_local_duckdb_idempotent_flag() ||
         (duckdb_commit_multi_trx_due_to_rotate_frequency &&
          ++rli->m_duckdb_skip_commit_due_to_rotate_cnt >=
              duckdb_commit_multi_trx_due_to_rotate_frequency))) {
      if (myduck::duckdb_log_options & LOG_DUCKDB_MULTI_TRX_BATCH_COMMIT) {
        myduck::log_duckdb_multi_trx_batch_commit("rotate");
      }
      rli->m_duckdb_skip_commit_due_to_rotate_cnt = 0;
      error = rli->info_thd->get_duckdb_context()->implicit_commit_batch();
      if (error) return error;
    }

    rli->set_global_duckdb_idempotent_flag(false);
  }
...
int Xid_apply_log_event::do_apply_event(Relay_log_info const *rli) {
  DBUG_TRACE;
  if (thd->get_duckdb_context(false) != nullptr &&
      thd->get_duckdb_context()->duckdb_delay_commit(this)) {
    return 0;
  }
...
void Rows_log_event::decide_row_lookup_algorithm_and_key() {
  ...
  */
  ...
  if (this->m_table->s->db_type()->db_type == DB_TYPE_DUCKDB) {
    this->m_rows_lookup_algorithm = ROW_LOOKUP_DUCKDB;
    return;
  }
...
int Rows_log_event::do_duckdb_update(Relay_log_info const *rli) {
  DBUG_TRACE;
  assert(m_table && m_table->in_use != nullptr);
  ...
  prepare_record(m_table, &this->m_local_cols, false);
  error = unpack_current_row(rli, &m_cols, false /*is not AI*/);
  assert(error == 0);

  // TODO: set read_set only primary index
  error = do_apply_row(rli);
  assert(error == 0);
  ...
  if (table) {
    ...
    */
    ...
    switch (m_rows_lookup_algorithm) {
      ...
      case ROW_LOOKUP_DUCKDB:
        do_apply_row_ptr = &Rows_log_event::do_duckdb_update;
        break;
...
int Table_map_log_event::do_apply_event(Relay_log_info const *rli) {
  ...
  if (rli->rpl_filter != nullptr) {
    const char* rewritten_tbl = rli->rpl_filter->get_rewrite_table(db_mem, tname_mem, &dummy_len);
    if (rewritten_tbl != nullptr) {
      const char *p= strchr(rewritten_tbl, '.');
      if (p != nullptr) {
        strmake(db_mem, rewritten_tbl, p - rewritten_tbl);
        my_stpcpy(tname_mem, p + 1);
        rli->rpl_filter->get_rewrite_wild_table_statistics()->increase_counter();
      }
    }
  }
...
bool Empty_log_event::write_data_body(Basic_ostream *ostream) {
  size_t data_len =
      m_size - Binary_log_event::IGNORABLE_HEADER_LEN - LOG_EVENT_HEADER_LEN;
...
uint32 Gtid_log_event::write_post_header_to_memory(uchar *buffer) {
  ...
  gtid_flags |=
      has_insert_only ? binary_log::Gtid_event::FLAG_HAS_INSERT_ONLY : 0;
...
int Gtid_log_event::do_apply_event(Relay_log_info const *rli) {
  ...
  */
  ...
  if (myduck::duckdb_log_options & LOG_DUCKDB_MULTI_TRX_BATCH_DETAIL) {
    myduck::log_duckdb_gtid("set gtid_next when apply Gtid_log_event", spec.type, spec.gtid.sidno,
        spec.gtid.gno);
  }
  ...
  */
  ...
  const_cast<Relay_log_info *>(rli)->set_local_duckdb_idempotent_flag(
      duckdb_idempotent_flag);
  const_cast<Relay_log_info *>(rli)->set_duckdb_insert_only_flag(
      has_insert_only);
  const_cast<Relay_log_info *>(rli)->set_transaction_length(transaction_length);
...
Transaction_context_log_event::Transaction_context_log_event(
    ...
    bool is_gtid_specified_arg, const Gtid_set *snapshot_version_arg)
  ...
  if (snapshot_version_arg) {
    snapshot_version->add_gtid_set(snapshot_version_arg);
  } else {
    /*
...
size_t Transaction_context_log_event::to_string(char *buf) const {
  DBUG_TRACE;
  size_t bytes = snprintf(
      buf, 256, "server_uuid=%s\tthread_id=%u\tsnapshot_version=", server_uuid,
      thread_id);
  return bytes + snapshot_version->to_string(buf + bytes);
}

void Transaction_context_log_event::claim_memory_ownership(bool claim) {
  my_claim(temp_buf, claim);
  my_claim(this, claim);
  /* When constructing 'sid_map' and 'snapshot_version', PSI_memory_key is
     not used to identify the memory they occupy (see 'Log_event::operator new'
     for how to do this). These two lines needs to be commented out, otherwise
     it will cause a crash in debug mode. */
...
int Transaction_context_log_event::pack_info(Protocol *protocol) {
  DBUG_TRACE;
  size_t length = snapshot_version->get_string_length();
  char *buf =
      (char *)my_malloc(key_memory_log_event, length + 256, MYF(MY_WME));
  size_t bytes = to_string(buf);
  protocol->store_string(buf, bytes, &my_charset_bin);
  my_free(buf);
  return 0;
}
...
int Transaction_context_log_event::do_apply_event(Relay_log_info const *rli) {
  DBUG_TRACE;
  assert(rli->info_thd == thd);
  assert(rli->info_thd->in_active_multi_stmt_transaction());

  rli->info_thd->get_duckdb_context()->add_gtid_set(snapshot_version);
...
bool Transaction_context_log_event::decode_snapshot_version() {
  DBUG_TRACE;
  assert(snapshot_version->is_empty());
