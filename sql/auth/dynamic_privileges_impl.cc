...
bool dynamic_privilege_init(void) {
  ...
  int ret = false;
  {
    ...
    if (service.is_valid()) {
      ...
      ret += service->register_privilege(STRING_WITH_LEN("AI_INVOKE"));
      ret += service->register_privilege(STRING_WITH_LEN("AI_ADMIN"));
      ret += service->register_privilege(STRING_WITH_LEN("AI_AUDIT_VIEWER"));
    }
  }  // exist scope
