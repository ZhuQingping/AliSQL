...
my_bool my_hash_init2(PSI_memory_key psi_key, HASH *hash, size_t growth_size,
  ...
  res = my_init_dynamic_array(&hash->array, psi_key, sizeof(HASH_LINK), nullptr,
                              size, growth_size);
  DBUG_RETURN(res);
}
...
void my_hash_free(HASH *hash) {
  ...
  DBUG_PRINT("enter", ("hash:%p  elements: %ld", hash, hash->records));
...
static inline char *my_hash_key(const HASH *hash, const uchar *record,
                                size_t *length, my_bool first) {
  if (hash->get_key) return (char *)(*hash->get_key)(record, length, first);
...
static inline uint my_hash_rec_mask(HASH_LINK *pos, size_t buffmax,
                                    size_t maxlength) {
...
uchar *my_hash_first_from_hash_value(const HASH *hash,
  ...
  if (hash->records) {
    uint flag = 1;
    uint idx = my_hash_mask(hash_value, hash->blength, hash->records);
    if (!length)
...
my_bool my_hash_insert(HASH *info, const uchar *record) {
  ...
  uchar *rec_data = nullptr, *rec2_data = nullptr, *key;
...
my_bool my_hash_update(HASH *hash, uchar *record, uchar *old_key,
  ...
  idx = my_hash_mask(
      hash->hash_function(hash->charset, old_key,
...
my_bool my_hash_iterate(HASH *hash, my_hash_walk_action action,
                        void *argument) {
  uint records, i;
  ...
  for (i = found = max_links = seek = 0; i < records; i++) {
    ...
    if (my_hash_rec_mask(data + i, blength, records) == i) {
      ...
      for (idx = data[i].next; idx != NO_RECORD && found < records + 1;
        ...
        hash_info = data + idx;
        seek += ++links;
        if ((rec_link = my_hash_rec_mask(hash_info, blength, records)) != i) {
