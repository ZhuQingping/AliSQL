...
> **Execution status:** 2026-07-30 已按本计划实现并验证；复核后的临时 Authorization header 会在同步调用结束后清零。
...
- [x] **Step 1: Write the failing compile contract**
...
- [x] **Step 2: Implement exact-row lookup**
...
- [x] **Step 3: Update runtime call sites**
...
- [x] **Step 4: Verify server link and registry regression**
...
- [x] **Step 1: Add a deterministic MTR case**

Insert a profile row with `credential_kind='PLAINTEXT_DEV'`, a harmless fixture value and
the valid bge-m3 1024 dimension. Use an incompatible local endpoint path and invoke
`AI_EMBEDDING`; a local adapter protocol rejection proves that the exact profile and
credential were read without sending HTTP. Record no fixture value.
...
- [x] **Step 1: Run full offline verification before any cloud request**
...
- [x] **Step 2: Configure disposable local profiles**
...
- [x] **Step 4: Remove temporary plaintext rows**
...
- [x] **Step 5: Record and commit sanitized evidence**
